    ...
    while offset < proximity_window:
        ...
        # Original logic for separate labels
            ...
            if p_match:
                ...
                if current_province and (annotations_cache.get(current_index, {}).get("label") != LABEL_LOC_P):
            ...
            if c_match:
                ...
                if current_city and (annotations_cache.get(current_index, {}).get("label") != LABEL_LOC_C):
            ...
            if b_match:
                ...
                if current_barangay and (annotations_cache.get(current_index, {}).get("label") != LABEL_LOC_B):
    ...
    while index < final_index:
        ...
        if found_headers and found_components:
            ...
            while index < final_index and age < MAX_AGE:
                ...
                # if barangay index is before province index, look for a province pa, and if we find, overwrite
                found_headers_in_row, header_indices_in_row, new_index_in_row, _ = find_column_headers(df, data,
                                                                                                       new_index_2,
                                                                                                       debug=debug_header)
                ...
                row_is_valid = (not ((pd.isnull(classification) or str(classification).strip() == '') and (
                        pd.isnull(zv) or str(zv).strip() == ''))) and str(
                    cleaned_row).strip()  # at least one of class/zv exists
                if not row_is_valid:
